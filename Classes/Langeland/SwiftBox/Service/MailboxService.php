...
class MailboxService {
	...
	function open($create = false) {
		...
		if (!is_file($this->_file)) {
			if ($create) {
				...
				throw new \Exception('Cannot open the mbox file "' . $this->_file . '": file does not exist.', MAIL_MBOX_ERROR_FILE_NOT_EXISTING);
			}
		}
		...
		if (!is_resource($this->_resource)) {
			throw new \Exception('Cannot open the mbox file: maybe without permission.', MAIL_MBOX_ERROR_NO_PERMISSION);
		}
	...
	function _create() {
		...
		//error
		throw new \Exception('File could not be created', MAIL_MBOX_ERROR_CANNOT_WRITE);
	}
	...
	function close() {
		if (!is_resource($this->_resource)) {
			throw new \Exception('Cannot close the mbox file because it was not open.', MAIL_MBOX_ERROR_NOT_OPEN);
		}

		if (!fclose($this->_resource)) {
			throw new \Exception('Cannot close the mbox, maybe file is being used (?)', MAIL_MBOX_ERROR_CANNOT_CLOSE);
		}
	...
	function get($message) {
		...
		if (!is_array($this->_index[$message])) {
			throw new \Exception('Message does not exist.', MAIL_MBOX_ERROR_MESSAGE_NOT_EXISTING);
		}
		...
		if (!is_resource($this->_resource)) {
			throw new \Exception('Mbox resource is not valid. Maybe you need to re-open it?', MAIL_MBOX_ERROR_NO_RESOURCE);
		}

		// seek to start of message
		if (fseek($this->_resource, $bytesStart) == -1) {
			throw new \Exception('Cannot read message bytes', MAIL_MBOX_ERROR_CANNOT_READ);
		}

		if ($bytesEnd - $bytesStart <= 0) {
			throw new \Exception('Message byte length is negative', MAIL_MBOX_ERROR_CANNOT_READ);
		}
	...
	function remove($message) {
		if ($this->hasBeenModified()) {
			throw new \Exception('File has been modified since loading. Re-open the file.', MAIL_MBOX_ERROR_MODIFIED);
		}
		...
		foreach ($message as $msg) {
			if (!isset($this->_index[$msg])
				...
				throw new \Exception('Message ' . $msg . 'does not exist.', MAIL_MBOX_ERROR_MESSAGE_NOT_EXISTING);
			}
		}
		...
		if ($ftemp === false) {
			throw new \Exception('Cannot create a temp file "' . $ftempname . '".', MAIL_MBOX_ERROR_CANNOT_CREATE_TMP);
		}
	...
	function update($message, $content) {
		if (!$this->_isValid($content)) {
			throw new \Exception('Message is invalid', MAIL_MBOX_ERROR_MSG_INVALID);}
		...
		$ftemp = fopen($ftempname, 'w');
		if ($ftemp === false) {
			throw new \Exception('Cannot create temp file "' . $ftempname . '" .', MAIL_MBOX_ERROR_CANNOT_CREATE_TMP);
		}
	...
	function append($content) {
		...
		if ($fp === false) {
			throw new \Exception('Cannot open file "' . $this->_file . '" for appending.', MAIL_MBOX_ERROR_CANNOT_OPEN);
		}

		if (fwrite($fp, $content, strlen($content)) === false) {
			throw new \Exception('Cannot write to file "' . $this->_file . '".', MAIL_MBOX_ERROR_CANNOT_WRITE);
		}
	...
	function _move($ftempname, $filename) {
		if (!copy($ftempname, $filename)) {
			throw new \Exception('Cannot copy "' . $ftempname . '" to "' . $filename . '".', MAIL_MBOX_ERROR_CANNOT_WRITE);
		}
	...
	function _process() {
		...
		// sanity check
		if (!is_resource($this->_resource)) {
			throw new \Exception('Resource is not valid. Maybe the file has not be opened?', MAIL_MBOX_ERROR_NOT_OPEN);
		}

		// going to start
		if (fseek($this->_resource, 0) == -1) {
			throw new \Exception('Cannot read mbox', MAIL_MBOX_ERROR_CANNOT_READ);
		}
	...
	function setTmpDir($tmpdir) {
		if (is_dir($tmpdir) && !is_writable($tmpdir)) {
			throw new \Exception('"' . $tmpdir . '" is not writable.', MAIL_MBOX_ERROR_CANNOT_WRITE);
		} else {
			$this->tmpdir = $tmpdir;
			return true;
		}
